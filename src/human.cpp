...
vector<pair<pair<int,int>,pair<int,int>>> Human::place_ships(){
    ...
    for(int i = 0;i < 5;i++){
        ...
        cout << "Enter the starting co-ordinates of the " << Player::ship_names[i] << " (Size = " <<  Player::ship_sizes[i] + 1 << ")"  << " separated by comma: ";
        getline(cin,buffer,'\n');
        ...
        cout << "Enter the ending co-ordinates of the " << Player::ship_names[i] << " separated by comma: ";
        getline(cin,buffer,'\n');
